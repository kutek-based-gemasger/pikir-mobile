...
class _PikirAppState extends ConsumerState<PikirApp>
  ...
  /// Opens the interception the screen watcher arrived with, if any.
  ///
  /// Pushed on top of whatever is showing, including the splash screen, which
  /// steps aside rather than replacing it.
  Future<void> _checkTrigger() async {
    ...
    if (trigger.isCheckout) {
      ...
      controller.startCheckout(
        amount: trigger.amount ?? 1250000,
        fromRealTrigger: true,
      );
      unawaited(navigator.pushNamed(Routes.intervensiBlanket));
    } else {
      controller.startLoanApp(fromRealTrigger: true);
